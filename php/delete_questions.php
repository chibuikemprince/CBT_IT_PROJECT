 ...
 
 {
	 ...
	 if(isset($_POST['action'])  ){
		...
		$uid  = $uid_ar[1];
		  ...
		  $check_if_question_already_registered =  mysqli_query($db_con,"SELECT * FROM questions WHERE id ='".$uid."'  ORDER BY id DESC LIMIT 1" );

if(mysqli_num_rows($check_if_question_already_registered)==1 ){
	
	
	 
	$sql=" DELETE FROM  `questions` WHERE  `id` ='".$uid."'";
	
	$update = mysqli_query($db_con,$sql);
	if($update){
		$response['success'] = " <br/> <h5 align='center' class='success'>Question has been deleted successfully. </h5><br/>";
